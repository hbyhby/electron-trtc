...
import { getBasicInfo } from '@/config/basicInfoConfig';
import router from '@/router';
import { useRoute } from 'vue-router';
import { Ref, ref, reactive, onMounted, onUnmounted } from 'vue';
import {useLocale} from '@/locales/useLocale';
import {  getTheme } from '@/utils/request/utils';

const localeHooks=useLocale()
...
async function handleInit() {
  ...
  conference.setLanguage(localeHooks.locale.value.replace('_','-') as LanguageOption);
...
const changeLanguage = (language: LanguageOption) => {
  localeHooks.changeLocale(language.replace('-', '_'));
